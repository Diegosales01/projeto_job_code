...
try:
    ...
    st.write(f"Erro ao baixar as stopwords: {e}")
...
try:
    ...
    dados = pd.DataFrame()  # Definir dados vazios caso haja erro
            ...
            if st.button(f"Selecionar Opção {i+1}", key=i):
                ...
                def registrar_feedback(descricao_usuario, codigo_escolhido):
                    ...
                    try:
                        ...
                        st.write(f"Erro ao registrar o feedback: {e}")

                # Registrar o feedback do usuário
                registrar_feedback(descricao_usuario, codigo_similar)
else:
    st.write("Erro ao carregar o arquivo. Por favor, tente novamente.")
